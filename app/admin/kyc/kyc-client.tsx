...
export default function KycClient({ initialUsers }: { initialUsers: User[] }) {
  ...
  async function loadDocs(userId: string) {
    ...
    const { data } = await supabase.storage.from('kyc-documents').list(`kyc/${userId}`, { limit: 10 })
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-6 space-y-6">
      <h1 className="font-serif font-bold text-2xl mb-2">KYC Verification</h1>
      ...
      {pending.length > 0 && (
        <div>
          ...
          <div className="space-y-3">
            {pending.map(u => (
              <KycCard key={u.id} user={u} docs={docUrls[u.id]} onLoadDocs={loadDocs} onApprove={approveKyc} onRevoke={revokeKyc} />
            ))}
          </div>
        </div>
      )}
              ...
              <div className="flex flex-wrap gap-2">
                {docs.map((url, i) => (
                  <a key={i} href={url} target="_blank" rel="noopener noreferrer"
                    className="flex items-center gap-1 text-xs text-forest-700 dark:text-forest-400 hover:underline bg-forest-50 dark:bg-forest-950 px-2 py-1 rounded">
                    <FileText className="w-3 h-3" />Document {i + 1}<ExternalLink className="w-3 h-3" />
                  </a>
                ))}
              </div>
            )}
